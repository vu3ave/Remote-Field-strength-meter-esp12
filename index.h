...
<h3>Field Strength Reading</h3>
<input type="text" id="analogdata" value=0 disabled>
<input type="text" id="pk" value=0 disabled>
<button class='button' onclick="getElementById('pk').value=0"> Reset Peak </button>

<h3>Sensitivity</h3>
<button class='button' onclick="addsen()"><b> + </b></button>
<input type="text" id="sen" value=0 disabled >
<button class='button' onclick="subsen()"><b> - </b></button>

<p></p>

<b type=text id="sen1" >0</b>
</div>

<script>
function addsen(){
document.getElementById("sen").value = parseInt(document.getElementById('sen').value)+100;
sendSen(document.getElementById("sen").value);
}
...
function getanalogdata(){
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("analogdata").value =
      this.responseText;
    }
  };
  xhttp.open("GET", "adata?mytxt=1", true);
  xhttp.send(); 
...
function sendSen(txt) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      
      document.getElementById("sen1").innerHTML ="<font color=blue><b> " + this.responseText +"</b></font>";
      
    }
  };
  xhttp.open("GET", "sendsen?mytxt="+txt, true);
  xhttp.send();
}
...
function peakfinder(){
  ...
  // giving data to gauge
  gauge.value = parseInt(document.getElementById("analogdata").value);

  
  if( document.getElementById("pk").value < parseInt(document.getElementById("analogdata").value))
  {
    document.getElementById("pk").value = parseInt(document.getElementById("analogdata").value);
  }
...
<script>
// remote fsm--interval timer
setInterval(getanalogdata, 100);

// data handling timer
setInterval(peakfinder, 50);
...
<button onclick="gdata()">Get Analog in A0 data</button>
